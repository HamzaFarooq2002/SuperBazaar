...
import { ArrowLeft, Gift, Star, TrendingUp, ShoppingBag, Zap, Award, Inbox } from 'lucide-react';
...
export function Rewards() {
  ...
  useEffect(() => {
    const loadRewards = async () => {
      try {
        const res = await api.users.getRewards();
        if (res.success && res.data) {
          setRewardStats({
            totalPoints: res.data.totalPoints ?? 0,
            cashbackEarned: res.data.cashbackEarned ?? 0,
            redeemedValue: res.data.redeemedValue ?? 0,
  ...
  const redeemOptions = [
    { id: '1', title: 'PKR 500 Voucher', points: 500, description: 'Use on your next purchase', image: '\u{1F4B0}' },
    { id: '2', title: 'Free Delivery', points: 200, description: 'Valid for 30 days', image: '\u{1F69A}' },
    { id: '3', title: 'PKR 1000 Voucher', points: 1000, description: 'Premium reward', image: '\u{1F381}' },
    { id: '4', title: '10% Discount', points: 300, description: 'On selected items', image: '\u{1F3F7}\u{FE0F}' }
  ];

  const iconForType = (type: string) => {
    if (type === 'Cashback') return { icon: Gift, color: 'green' };
    if (type === 'Bonus') return { icon: Zap, color: 'purple' };
      ...
      {loading ? (
        ...
        <div className="px-6 mt-6">
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-white text-[32px]">{rewardStats.totalPoints.toLocaleString()}</p>
              </div>
              ...
              {redeemOptions.map((option, index) => (
                ...
                >
                  ...
                  {rewardStats.totalPoints >= option.points ? (
                    ...
                    <p className="text-gray-400 text-xs text-center mt-3">Not enough points</p>
                  )}
                </motion.div>
              ))}
            </div>
            <p className="text-gray-400 text-xs text-center mt-3">Redemption features coming soon</p>
          </motion.div>
          ...
          >
            ...
            {rewardHistory.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-400 text-sm">No rewards yet. Start shopping to earn!</p>
              </div>
            ) : (
              <div className="space-y-3">
                {rewardHistory.map((reward: any, index: number) => {
                  const { icon: Icon, color } = iconForType(reward.type);
                  return (
                    <motion.div
                      key={reward.id || index}
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          <p className="text-green-600">+{(reward.amount ?? 0).toLocaleString()}</p>
                        </div>
                      </div>
                    </motion.div>
                  );
          ...
          >
            ...
            <p className="text-white/90 text-sm mb-4">Get 50 points for every order placed</p>
            <button 
